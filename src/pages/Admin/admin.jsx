      ...
      <CategoriesList isVisible={isCategoriesVisible} />
        ...
        {formData.starring.map((star, index) => (
          <div style={{marginTop: '10px', display: 'flex'}} key={index}>
            <input style={{width: '49%'}} className={`txtDM ${darkMode ? 'dark-mode' : ''}`} type="text" name="name" value={star.name} onChange={(e) => handleStarringChange(index, e)} placeholder={`Actor Name ${index + 1}`} required />
